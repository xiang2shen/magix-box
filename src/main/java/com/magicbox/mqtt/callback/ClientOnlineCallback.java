...
@Component
@Slf4j
public class ClientOnlineCallback extends AbstractMqttCallback {
	...
	public void callback(String topic, String message) {
		...
		if (StringUtils.isNotBlank(message) && message.contains(CLIENT_ID)) {
			...
			if (null != map) {
				...
				if (null != frame && FrameStatusEnum.OFFLINE.getCode().equals(frame.getFrameStatus())) {
					
					log.info("clientId={}已上线", frameCode);
					frameService.updateFrameStatus(frame.getId(), FrameStatusEnum.ONLINE);
				}
			}
		}
	}
